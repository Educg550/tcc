...
| `models/dominio.py` | `Requisito`, `Alvo`, `Projeto`, `Escopo`, `Modo` — o mundo em que o modelo age |
...
| `models/agentes.py` | agentes Agno e o `Avaliador` (CUA via browser-use) |
| `models/politicas.py` | `Orcamento` (passos, custo, tempo) e `Permissao` (`Batch`/`Interativa`) |
| `models/tracing.py` | `Trace` e o `Resultado` que vira `RUN.log` |
| `prompts/*.md` | todo texto de prompt. Prompt novo = arquivo novo, nunca string no código |
